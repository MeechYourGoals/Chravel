...
import { SystemMessageCategoryPrefs, DEFAULT_SYSTEM_MESSAGE_CATEGORIES } from '@/utils/systemMessageCategory';
...
const CATEGORY_LABELS: Record<keyof SystemMessageCategoryPrefs, { label: string; description: string }> = {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400">
            Display system messages for trip activity
          </p>
        </div>
        <Switch
          id="show-activity"
      ...
      {showSystemMessages && (
        <div className="space-y-3 pl-2 border-l-2 border-white/10 ml-1">
          {(Object.keys(CATEGORY_LABELS) as Array<keyof SystemMessageCategoryPrefs>).map((category) => {
      ...
      <p className="text-xs text-gray-500 italic">
        This only affects what you see inside chat. Alerts, export, and event log still include all activity.
      </p>
    </div>
  );
};
