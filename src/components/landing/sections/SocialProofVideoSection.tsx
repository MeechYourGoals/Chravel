...
export const SocialProofVideoSection = () => {
  return (
    <div className="container mx-auto px-4 py-8 tablet:py-0 flex flex-col items-center justify-start tablet:justify-center min-h-0 tablet:min-h-screen space-y-6 tablet:space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 max-w-6xl w-full px-2">
        {metrics.map((metric, index) => (
          <Card
            key={index}
            className="bg-card/50 backdrop-blur-sm border border-border/50 hover:border-primary/50 transition-all duration-300 w-full overflow-hidden"
          >
            <CardContent className="p-3 sm:p-4 space-y-1">
              {/* Inline icon + value */}
              <div className="flex items-center justify-center gap-2">
                <div className="text-primary flex-shrink-0">{metric.icon}</div>
