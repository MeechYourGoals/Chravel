...
CREATE TABLE public.campaigns (
  ...
  destination_info JSONB, -- {location, city, country, etc}
  tags TEXT[] DEFAULT '{}',
...
CREATE TABLE public.campaign_targeting (
  ...
  age_min INTEGER,
  age_max INTEGER,
  genders TEXT[] DEFAULT '{}', -- ['male', 'female', 'other', 'all']
  interests TEXT[] DEFAULT '{}', -- ['nightlife', 'food_dining', 'adventure', etc]
  locations TEXT[] DEFAULT '{}', -- Cities/regions
  trip_types TEXT[] DEFAULT '{}', -- ['leisure', 'business', 'group', 'family']
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(campaign_id)
);

-- Create campaign analytics table for tracking impressions/clicks
...
-- RLS Policies for campaigns (advertisers can manage their own)
...
-- RLS Policies for campaign targeting
...
-- RLS Policies for analytics (write-only for system, read for advertisers)
CREATE POLICY "analytics_insert_any"
  ON public.campaign_analytics FOR INSERT
  WITH CHECK (true); -- Anyone can track events
...
-- Storage policies for advertiser-assets bucket
