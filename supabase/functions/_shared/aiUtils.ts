...
export function analyzeQueryComplexity(
  ...
  const reasoningKeywords = [
    'why', 'how', 'explain', 'analyze', 'compare', 'evaluate', 'reasoning',
    'strategy', 'optimize', 'best approach', 'what if', 'scenario',
    'trade-off', 'pros and cons', 'consider', 'think about'
  ];
  
  const multiStepKeywords = [
    'then', 'after', 'next', 'step', 'process', 'workflow', 'sequence',
    'first', 'second', 'finally', 'subsequently', 'followed by'
  ];
  
  const technicalTerms = [
    'api', 'integration', 'algorithm', 'architecture', 'optimization',
    'scalability', 'performance', 'latency', 'throughput', 'database',
    'query', 'index', 'cache', 'async', 'concurrent', 'distributed'
  ];
  
  const requiresContextKeywords = [
    'based on', 'according to', 'from the', 'in the context', 'given that',
  ...
  const totalScore = lengthFactor + reasoningFactor + multiStepFactor + 
                     contextFactor + technicalFactor + historyFactor + contextSizeFactor;
...
export function requiresChainOfThought(query: string, complexity: QueryComplexity): boolean {
  const chainOfThoughtKeywords = [
    'should', 'recommend', 'best', 'compare', 'evaluate', 'analyze',
    'strategy', 'optimize', 'consider', 'trade-off', 'pros and cons'
  ];
