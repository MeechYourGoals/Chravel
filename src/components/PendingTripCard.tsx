...
import { CalendarRange, MapPin, Clock, AlertCircle } from 'lucide-react';
...
export const PendingTripCard = ({ trip }: PendingTripCardProps) => {
  ...
  return (
    <div
      className="group bg-white/5 hover:bg-white/10 backdrop-blur-xl border border-white/10 hover:border-gold-primary/30 rounded-2xl md:rounded-3xl overflow-hidden transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl shadow-lg md:shadow-black/20 opacity-60 cursor-not-allowed"
      onClick={handleClick}
    >
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            <div className="flex items-start gap-3 mb-2">
              <div className="flex-1">
                <h3 className="text-lg md:text-xl font-bold text-white/80 group-hover:text-gold-primary/80 transition-colors line-clamp-2">
                  {trip.title}
                </h3>
                ...
                <div className="mt-2 flex items-center gap-2">
                  <Badge
                    variant="secondary"
                    className="bg-gold-primary/20 text-gold-primary border-gold-primary/30 flex items-center gap-1"
                  >
            ...
            <div className="flex items-center gap-2 text-white/60 text-sm md:text-base">
              <CalendarRange size={14} className="md:hidden gold-gradient-icon opacity-60" />
              <CalendarRange size={18} className="hidden md:block gold-gradient-icon opacity-60" />
      ...
      <div className="p-4 md:p-6">
        ...
        {/* Pending Message */}
        <div className="mb-4 md:mb-6 p-3 md:p-4 bg-gold-primary/10 border border-gold-primary/20 rounded-xl">
          <div className="flex items-start gap-2">
            <AlertCircle className="h-4 w-4 md:h-5 md:w-5 text-gold-primary mt-0.5 flex-shrink-0" />
            <div className="flex-1">
              <p className="text-xs md:text-sm text-gold-primary/90 font-medium mb-1">
                Waiting for organizer approval
              </p>
              <p className="text-xs text-gold-primary/70">
