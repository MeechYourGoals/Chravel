...
import type { OptimisticMessage, MessageStatus, RichChatAttachment, RichLinkPreview } from '@/types/chatAttachment';
...
export const useOptimisticMessages = ({ tripId }: UseOptimisticMessagesOptions) => {
  ...
  const persistFailedMessages = useCallback((messages: OptimisticMessage[]) => {
    try {
  ...
  const markAsSent = useCallback((clientMessageId: string) => {
    setPendingMessages(prev => {
  ...
  const isPending = useCallback((clientMessageId: string): boolean => {
    return pendingMessages.some(msg => msg.clientMessageId === clientMessageId);
  }, [pendingMessages]);

  // Get pending message by clientMessageId
  const getPendingMessage = useCallback((clientMessageId: string): OptimisticMessage | undefined => {
