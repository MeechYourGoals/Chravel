...
export const useRolePermissions = (tripId: string) => {
  ...
  const loadPermissions = useCallback(async () => {
    ...
    try {
      // Check trip membership and trip creator status in parallel
      ...
      const isUserTripMember = !!memberResult.data;
      const isTripCreator = tripResult.data?.created_by === user.id;
      setIsTripMember(isUserTripMember || isTripCreator);

      // Trip creator gets full admin permissions
      if (isTripCreator) {
        setPermissionLevel('admin');
        setFeaturePermissions({
          channels: {
  ...
  const canPerformAction = useCallback(
    (feature: keyof FeaturePermissions, action: string): boolean => {
      ...
      // While permissions are still loading, allow actions optimistically.
      // Supabase RLS enforces actual data-level security server-side,
      // so this prevents false permission denials during the async load.
    ...
    [featurePermissions, isDemoMode, isLoading, isTripMember],
  );
