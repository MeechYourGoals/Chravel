...
test.describe('Trip Creation', () => {
  test('TRIP-001-TC01: Create trip via modal', async ({ page, createTestUser, loginAsUser }) => {
    ...
    const destinationInput = page
      .locator('input[name="destination"], input[placeholder*="destination"]')
      .first();
...
test.describe('Trip Viewing', () => {
  test('TRIP-002-TC01: User sees their trips on home', async ({
    ...
    await expect(page.locator(`text=Visible Trip`).first()).toBeVisible({ timeout: 10000 });
  });
  ...
  test('TRIP-003-TC01: Can view trip detail page', async ({
    ...
    const tabsVisible = await page
      .locator('[data-testid="chat-tab"], [data-testid="calendar-tab"], text=Chat, text=Calendar')
      .first()
...
test.describe('Trip Editing', () => {
  test('TRIP-004-TC01: Owner can edit trip name', async ({
    ...
    const infoButton = page
      .locator('[data-testid="trip-info-button"], button:has-text("Info"), button:has-text("More")')
      .first();
    ...
    if (await infoButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const editButton = page
        .locator('[data-testid="edit-trip"], button:has-text("Edit"), text=Edit')
        .first();
      ...
      if (await editButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const nameInput = page
          .locator('input[name="name"], input[value="Original Trip Name"]')
          .first();
    ...
    // Verify in database
    const { data } = await supabaseAdmin.from('trips').select('name').eq('id', trip.id).single();
...
test.describe('Trip Archive', () => {
  test('TRIP-008-TC01: Owner can archive trip', async ({
    ...
    const menuButton = page
      .locator('[data-testid="trip-menu"], button[aria-label="Options"], button:has(svg)')
      .first();
    ...
    if (await menuButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const archiveOption = page
        .locator('button:has-text("Archive"), [data-testid="archive-trip"]')
        .first();
      ...
      if (await archiveOption.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const confirmButton = page
          .locator('button:has-text("Confirm"), button:has-text("Archive")')
          .first();
  ...
  test('TRIP-009-TC01: Can view archived trips page', async ({
    ...
    await supabaseAdmin.from('trips').update({ is_archived: true }).eq('id', trip.id);
...
test.describe('Trip Search and Filter', () => {
  test('TRIP-014-TC01: Can search trips', async ({
    ...
    const searchButton = page
      .locator('[data-testid="search-button"], button[aria-label="Search"], button:has(svg)')
      .first();
...
test.describe('Trip Member Access', () => {
  test('TRIP-003-TC02: Non-member cannot access trip', async ({
    ...
    const blocked = await Promise.race([
      page
        .waitForSelector('text=not found, text=access denied, text=Trip Not Found', {
          timeout: 5000,
        })
        .then(() => true),
      page
        .waitForURL(url => !url.pathname.includes(`/trip/${trip.id}`), { timeout: 5000 })
        .then(() => true),
    ]).catch(() => false);
