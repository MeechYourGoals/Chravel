...
export class MockEmbeddingService {
  ...
  private static getMockEmbeddings(tripId: string): MockEmbedding[] {
    return [
      ...
      },
      {
        ...
        metadata: {
          location: 'The Little Nell Restaurant',
          start_time: new Date(
      ...
      {
        content_text:
          'Broadcast [emergency]: EMERGENCY: Severe weather warning in effect. Stay indoors until further notice!',
        source_type: 'broadcast',
  ...
  private static generateQueryEmbedding(query: string): number[] {
    ...
    if (
      ...
      lowercaseQuery.includes('complete')
    ) {
    ...
    if (
      ...
      lowercaseQuery.includes('decide')
    ) {
    ...
    if (
      lowercaseQuery.includes('urgent') ||
      lowercaseQuery.includes('emergency') ||
      lowercaseQuery.includes('important')
    ) {
