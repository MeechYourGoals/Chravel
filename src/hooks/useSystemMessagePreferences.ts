...
export function useGlobalSystemMessagePreferences() {
  ...
  const { data: preferences, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<SystemMessagePreferences> => {
      ...
      return {
        ...
        categories: { ...DEFAULT_SYSTEM_MESSAGE_CATEGORIES, ...(data.system_message_categories || {}) }
      };
    },
    enabled: !!user?.id
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: 'Preferences saved', description: 'Your chat activity settings have been updated.' });
    },
    onError: () => {
...
export function useTripSystemMessagePreferences(tripId: string) {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['effectiveSystemMessagePrefs', tripId, user?.id] });
      toast({ title: 'Trip preferences saved' });
    },
    onError: () => {
      toast({ title: 'Error', description: 'Failed to save trip preferences.', variant: 'destructive' });
    }
  });
  ...
  const resetMutation = useMutation({
    ...
    onError: () => {
      toast({ title: 'Error', description: 'Failed to reset preferences.', variant: 'destructive' });
    }
  });
    ...
    queryFn: async (): Promise<SystemMessagePreferences> => {
      ...
      if (tripPrefs) {
        return {
          ...
          categories: { ...DEFAULT_SYSTEM_MESSAGE_CATEGORIES, ...(tripPrefs.system_message_categories || {}) }
        };
      }
      ...
      if (globalPrefs) {
        return {
          ...
          categories: { ...DEFAULT_SYSTEM_MESSAGE_CATEGORIES, ...(globalPrefs.system_message_categories || {}) }
        };
      }
