...
import { searchMessages, searchMessagesByAuthor, searchMessagesByDateRange } from '@/services/messageSearchService';
...
export const useMessageSearch = ({ tripId, localMessages = [], isDemoMode = false }: UseMessageSearchOptions) => {
  ...
  const searchByAuthor = useCallback(async (authorName: string) => {
    setIsSearching(true);
    try {
      let searchResults;
