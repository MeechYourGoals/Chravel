...
| **Google Cloud TTS (concierge playback)** | Google Cloud | `GOOGLE_CLOUD_TTS_API_KEY` | `concierge-tts` (provider swapped to Google TTS) | Google Cloud Console/API key. Set via `supabase secrets set GOOGLE_CLOUD_TTS_API_KEY=...` and redeploy `concierge-tts`. | Y — if omitted, voice playback fails (text concierge still works) |
