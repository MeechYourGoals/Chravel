...
function calculateRelevance(query: string, response: string): number {
  ...
  const hasNonAnswer = nonAnswerPatterns.some(pattern => responseLower.includes(pattern));
...
function calculateCompleteness(query: string, response: string): number {
  ...
  return Math.min(
    1,
    lengthScore * 0.7 + (hasMultipleParts || hasMultipleQuestions ? 0.3 : 0.5) + structureBonus,
  );
}
...
function calculateSafety(response: string): number {
  ...
  const harmfulPatterns = [/\b(kill|murder|violence|harm|dangerous)\b/gi];
