...
serve(async req => {
  ...
  try {
    ...
    try {
      ...
      // Request body redacted for privacy
    ...
    switch (endpoint) {
      case 'embed-url': {
        ...
        const response = new Response(JSON.stringify({ embedUrl }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      ...
      case 'text-search': {
        ...
        // Search request redacted from logs
        ...
        // Text search response logged at debug level only
      ...
      case 'embed-with-origin': {
        ...
        const result = new Response(JSON.stringify({ embedUrl }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
