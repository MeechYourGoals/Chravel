...
test.describe('Authentication Flow - Happy Paths', () => {
  ...
  test('AUTH-001-TC01: Email/password signup creates account', async ({
    ...
    const signupTab = page.locator('text=Sign Up, text=Create Account, [data-testid="signup-tab"]').first();
    ...
    const result = await Promise.race([
      page.waitForURL(url => !url.pathname.includes('/auth'), { timeout: 10000 })
        .then(() => 'redirected'),
      page.waitForSelector('text=check your email, text=confirm', { timeout: 10000 })
  ...
  test('AUTH-008-TC01: Logout clears session', async ({
    ...
    const isProtected = await Promise.race([
      page.waitForURL(url => url.pathname.includes('/auth'), { timeout: 5000 })
        .then(() => true),
      page.waitForSelector('[data-testid="auth-modal"], text=Sign In, text=Log In', { timeout: 5000 })
        .then(() => true),
    ]).catch(() => false);
...
test.describe('Authentication Flow - Error States', () => {
  ...
  test('AUTH-009-TC02: Empty form shows validation', async ({ page }) => {
    ...
    const hasValidation = await page.locator(
      '[aria-invalid="true"], :invalid, text=required, text=enter'
...
test.describe('Authentication Flow - Session Management', () => {
  ...
  test('AUTH-011-TC01: Protected routes redirect unauthenticated users', async ({ page }) => {
    ...
    const redirectedOrPrompted = await Promise.race([
      page.waitForURL(url => url.pathname.includes('/auth') || url.pathname === '/', { timeout: 5000 })
        .then(() => true),
      page.waitForSelector('[data-testid="auth-modal"], text=Sign In, text=Log In', { timeout: 5000 })
        .then(() => true),
    ]).catch(() => false);
