...
Deno.serve(async req => {
  ...
  try {
    ...
    if (!isSuperAdmin) {
      ...
      const allowedTiers = ['frequent-chraveler', 'pro-starter', 'pro-growth', 'pro-enterprise'];
    ...
    const xaiResponse = await fetch('https://api.x.ai/v1/realtime/sessions', {
      ...
      body: JSON.stringify({
        ...
        instructions:
          'You are Chravel AI Concierge. Be concise, travel-smart, and action-oriented. Prefer actionable answers and bullets. Keep responses under 30 seconds of speech.',
