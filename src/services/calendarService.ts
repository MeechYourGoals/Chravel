...
export const calendarService = {
   ...
   * This avoids PostgREST timeouts and response-size issues
   * that occur with large single-request bulk inserts.
   */
  async bulkCreateEvents(events: CreateEventData[]): Promise<{
    ...
    console.log(`[calendarService] Importing ${rows.length} events for trip ${tripId}`);

    // 5. For small batches (≤5), use a single insert (proven to work)
    if (rows.length <= 5) {
      const { data, error } = await supabase.from('trip_events').insert(rows).select('*');

      if (!error && data && data.length > 0) {
        console.log(`[calendarService] Single insert success: ${data.length} events`);
      ...
      // Fall through to batched approach if single insert failed
      console.warn(
        '[calendarService] Single insert returned no data, falling back to individual inserts',
      );
    }

    // 6. For larger batches (>5) or failed single insert, use sequential batches
  ...
  async batchInsertEvents(
    ...
    console.log(
      `[calendarService] Batch inserting ${rows.length} events in groups of ${BATCH_SIZE}`,
    );
    ...
    // Cache successfully imported events in background
    ...
    console.log(
      `[calendarService] Batch import complete: ${imported} imported, ${failed} failed out of ${rows.length} total`,
    );
