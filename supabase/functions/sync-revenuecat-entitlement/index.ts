...
interface SyncRequest {
  customerInfo: {
    ...
    entitlements: {
      active: Record<string, { isActive: boolean; expirationDate: string | null; periodType?: string }>;
    };
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[sync-rc] Active entitlements:', Object.keys(customerInfo.entitlements?.active || {}));
    ...
    for (const [entitlementId, info] of Object.entries(activeEntitlements)) {
      if (info.isActive) {
        ...
        if (mappedPlan) {
          ...
          const planPriority = ['free', 'explorer', 'frequent-chraveler', 'pro-starter', 'pro-growth', 'pro-enterprise'];
